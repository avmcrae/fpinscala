...
object List {
  ...
  def productWithFoldLeft(numbers: List[Int]): Int = {
    if (numbers == Nil) return 0
    foldLeft(numbers, 1)((x, y) => x * y)
  }

  def lengthWithFoldLeft[A](list: List[A]): Int = {
  ...
  def concat[A](list: List[List[A]]): List[A] = {
    foldLeft(list, List[A]())((x, y) => append(x, y))
  }
  ...
  def flatMap[A](l: List[A])(f: A => List[A]): List[A] = {
    foldLeft(l, List[A]())((x, y) => append(x, f(y)))
  }

  def filterWithFlatMap[A](l: List[A])(f: A => Boolean): List[A] = {
    def filter(x: A): List[A] = { if (f(x)) List(x) else Nil }
    flatMap(l)(filter)
  }
